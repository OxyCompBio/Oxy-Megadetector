...
parser = argparse.ArgumentParser(
    description='Run MegaDetector on new upload folder photos')
parser.add_argument('photo_folder', type=str, help='Path to upload folder')
parser.add_argument('md_output', type=str,
                    help='Path to MegaDetector json output file destination')
...
def main(PHOTO_UPLOAD, md_output_dest):
  for dirpath, dirnames, filenames in os.walk(PHOTO_UPLOAD):
    ...
    for i in dirnames:
      ...
      subprocess.call(["python", "/home/compbio/megadetector/CameraTraps/detection/run_tf_detector_batch.py", detector_file,
                      current_photo_folder, output_file, "--recursive", "--output_relative_filenames"])
