...
parser = argparse.ArgumentParser(description = 'Update CSV file with megadetector detections and categories')
parser.add_argument('detector_json', type = str, help = 'Path to megadetector output file')
parser.add_argument('input_csv', type = str, help = 'CSV file to be updated with megadetector detections')
parser.add_argument('output_path', type = str, help = 'Desired path to updated CSV - include name')
...
def update_csv(detector_output, input_csv, output_path):
	...
	with open (input_csv, 'r') as read, open (output_path, 'w') as write, open (detector_output) as ref:
		...
		csv_writer.writerow(['photodir','isEmpty','hasExif','successfulOCR','OCRtimeStamp','exifTimeStamp','lastModifiedTimeStamp','detections catgegory/conf'])
