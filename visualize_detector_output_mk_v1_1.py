...
def visualize_detector_output(detector_output_path: str,
    ...
    # creating directories for each detection category
    animal_dir = out_dir + '/animal'
    person_dir = out_dir + '/person'
    vehicle_dir = out_dir + '/vehicle'
    no_detection = out_dir + '/no detection'
    ...
    for entry in tqdm(images):
        ...
        # Sort images -mk
        if category == '1':
          annotated_img_path = os.path.join(animal_dir, f'anno_{image_id}')
        elif category == '2':
          annotated_img_path = os.path.join(person_dir, f'anno_{image_id}')
        elif category == '3':
          annotated_img_path = os.path.join(vehicle_dir, f'anno_{image_id}')
        elif category == '0':
          annotated_img_path = os.path.join(no_detection, f'anno_{image_id}')
        else:
