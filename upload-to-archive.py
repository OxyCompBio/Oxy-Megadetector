...
proc = subprocess.Popen(["rclone", "lsf", "-R", "--files-only", "gdrive:UWINUpload"], stdout=subprocess.PIPE)
...
for file in out:
        with rate_limiter:
                if file != "":
                        ...
                        filename = dir_extList[0] + "_ArchivedAt-" + timeStr + date + "." + dir_extList[1]
                        ...
                        subprocess.Popen(["rclone", "copyto", "--no-check-dest", "--retries-sleep=2s", src, dst])
