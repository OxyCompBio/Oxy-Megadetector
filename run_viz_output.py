...
parser = argparse.ArgumentParser(
    description='Run visualize_detector_output.py on new upload folder photos')
parser.add_argument(
  ...
  for dirpath, dirnames, filenames in os.walk(detector_output_path):
    ...
    for i in filenames:
      ...
      if datetime.now() - datetime.fromtimestamp(os.stat(detector_output_file).st_ctime) < timedelta(14, 0, 0, 0, 0, 0):
        subprocess.call(["python", "visualize_detector_output.py", detector_output_file,
                        output_dir_current, "-c" + str(confidence), "-i" + photo_dir])


if __name__ == "__main__":
  main(args.detector_output_folder, args.out_dir,
       args.photo_upload_base_path, args.confidence)
